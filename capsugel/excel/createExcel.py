...
def write_to_excel(json_string):
    ...
    values1 = [
        ...
        # Safely handle variables and avoid NoneType errors
        (name.strip() if name is not None else '') if 'name' in locals() else '',
        (street.strip() if street is not None else '') if 'street' in locals() else '',
        (code_postal.strip() if code_postal is not None else '') if 'code_postal' in locals() else '',
        (city.strip() if city is not None else '') if 'city' in locals() else '',
        (country.strip() if country is not None else '') if 'country' in locals() else '',
        (term.strip() if term is not None else '') if 'term' in locals() else '',
        (place.strip() if place is not None else '') if 'place' in locals() else '',
        data.get('Container', ''),
        data.get('wagon', ''),
